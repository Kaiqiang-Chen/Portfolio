...
class XinlangNewsInfoSpider:
    ...
    def save_results_to_mongodb(self):
        ...
        db = client['xinlangcaijing']
        collection = db['nandu']

        for result in self.results:
            collection.insert_one(result)  # 将结果插入到MongoDB集合中

    def save_results_to_file(self, filename="xinlang_gegu_results_final.txt"):
...
if __name__ == "__main__":
    ...
    with open("nandu_links.txt", "r", encoding="utf-8") as file:
        start_urls = [line.strip() for line in file.readlines()][:3]
