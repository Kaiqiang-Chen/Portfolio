...
# 主函数：GA算法框架
def GA(filepath, Iterations, salesmen):
    run_start = time.perf_counter()
    model = Model()  # 初始化model
    model.salemen = salesmen
...
#     filepath = r"A-n80-k10.vrp"
#     model = GA(filepath, Iterations=20000)
#     print("-------------------------------------")
#     print("最短里程：", model.best_sol.obj)
#     print("最短车辆路径方案：", model.best_sol.Route)

if __name__ == '__main__':
    filepath = r'A-n32-k5.vrp'
    run_time = 8
    ran = range(3,11)
    i = 0
    all_best_obj = []
    ...
    while i < run_time:
        ...
        model = GA(filepath, Iterations=20000, salesmen = ran[i])
