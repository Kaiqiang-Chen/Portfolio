...
def splitRoutes(chrom, model):
    ...
    for i in range(0,len(chrom)):
        ...
        j = chrom[i]
        if j < 0:
            if len(route) >= 2 or i == len(chrom)-1:
                Routes.append(route.copy())
                route = []
            else:
                if chrom[i+1] != -1:
                    skip = 1
                    route.append(chrom[i+1])
                    Routes.append(route.copy())
                    route = []
                else:
                    Routes.append(route.copy())
    ...
    if len(Routes[0]) <= 1:
        if len(Routes[1]) >= 3:
            Routes[0].insert(len(Routes[0]), Routes[1][0])
            Routes[1].pop(0)
        else:
            Routes[0].insert(0, Routes[2][-1])
            Routes[2].pop()
    if len(Routes[1]) <= 1:
        if len(Routes[0]) >= 3:
            Routes[1].insert(0, Routes[0][-1])
            Routes[0].pop()
        else:
            Routes[1].insert(len(Routes[1]), Routes[2][0])
            Routes[2].pop(0)
    if len(Routes[2]) <= 1:
        if len(Routes[0]) >= 3:
            Routes[2].insert(len(Routes[2]), Routes[0][0])
            Routes[0].pop(0)
        else:
            Routes[2].insert(0, Routes[1][-1])
            Routes[1].pop()
    fu = [-1,-2]
    for i in range(3):
        nodes_seq += Routes[i]
        if i != 2:
            nodes_seq.append(fu[i])
    return No_vehicle, Routes, nodes_seq
...
def calObj(nodes_seq, model):
    routes_dis = []
    new_node = []
    num_vehicle, vehicle_routes, new_node = splitRoutes(nodes_seq, model)
    distance = 0
    for route in vehicle_routes:
        distance += calDistance(route, model)
        routes_dis.append(calDistance(route, model))
    return distance, vehicle_routes, routes_dis, new_node
...
def run(filepath,T0,Tf,detaT,v_cap,opt_type):
    ...
    sol.obj,sol.routes,sol.routes_dis,sol.nodes_seq=calObj(sol.nodes_seq,model)
    ...
    model.tabu_list=[[[0]],[0]]
    #开始迭代求解
    while Tk>=Tf and time.time() - start_time < 20:
        # 构建邻域算子列表
        ...
        for i in range(len(action_list)):
            ...
            if action_list[i] not in model.tabu_list[0]:
                ...
                new_sol.obj,new_sol.routes,new_sol.routes_dis,new_sol.nodes_seq=calObj(new_sol.nodes_seq,model)
                # 若新解优于当前局部最优解，进行更新
